...
public:
	...
	public:
		...
		SOCKET SocketConnectTimeout(const char* ip, int port, double timeout)
		{
			...
			if (select((int)sock + 1, NULL, &ws, NULL, &tv) > 0)
			{
				int res = ERROR;
				int len = sizeof(res);
	...
	protected:
		int parse(const char* msg, int len)
		{
			...
			if (*msg == '*')
			{
				...
				while (cnt > 0)
				{
					end = parseNode(str, (int)(tail - str));
	...
	public:
		...
		int getResult(RedisConnect* redis, int timeout)
		{
			auto doWork = [&]() {
				...
				if (sock.write(msg.c_str(), (int)msg.length()) < 0) return NETERR;
...
public:
	...
	bool connect(const string& host, int port, int timeout = 3, int memsz = 1024 * 1024)
	{
		...
		int val = sock.connect(host, port, timeout);
