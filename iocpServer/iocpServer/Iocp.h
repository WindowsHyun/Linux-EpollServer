...
public:
	...
	void initClient();
	void destroyThread();
	void add_tempUniqueNo(unsigned_int64 uniqueNo);
	bool BindRecv(class PLAYER_Session* pPlayerSession, int remainSize);			// WSARecv Overlapped I/O 작업을 시킨다.
	bool SendPacket(unsigned_int64 uniqueNo, char* pMsg, int nLen);				// Packet Send 처리를 한다.
...
private:
	...
	unsigned_int64 uniqueNo;											// 접속 uniqueNo
	...
	void ClosePlayer(unsigned_int64 uniqueNo);									// Socket 연결을 끊는다.
